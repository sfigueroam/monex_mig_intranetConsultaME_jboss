...
// @param expandOnServer true if this node is expecting to be expanded on the server
// @param ajaxPrefix the request path excluding the command
// @param ajaxParam the parameter value that will be added to the end of the request
NetUITree.prototype.getTreeCommandUrl = function(command,treeName,nodeName,expandOnServer,ajaxPrefix, ajaxParam)
{
    var url = ajaxPrefix + "/" + command + NetUI.prototype.xmlHttpRequestMapping +
        "?tree=" + treeName + "&node=" + nodeName;
    if (expandOnServer == "true") {
	    url = url + "&expandOnServer=true";
    }
    if (ajaxParam != null) {
        url = url + "&netuiajax=" + ajaxParam;
    }
    return url;
}
...
function NetUITreeXmlHttpRequestReturn(req)
{
   ...
   var treeName = req.responseXML.getElementsByTagName("treeId");
   ...
   treeName = treeName[0].childNodes[0].nodeValue;
   ...
   for (var i=0;i<document.links.length;i++) {
	   ...
	   if (attr == nodeName) {
	       ...
	       var x = netUI.netUITree.getTreeName(treeNode);
	       // verify that we are expanding the proper tree
	       if (x != treeName)
		   continue;
...
function NetUIExpandTree()
{
    ...
    var ajaxPrefix = netUI.netUITree.trees[treeName].ajaxPrefix;
    var ajaxParam =  netUI.netUITree.trees[treeName].ajaxParam;

    var url = netUI.netUITree.getTreeCommandUrl(cmd,treeName,nodeName,
		 expandOnServer,ajaxPrefix, ajaxParam);
    ...
    var expandLast = netUIGetAttribute(this,"netui","expandLast");
    var expandImage = netUIGetAttribute(this,"netui","imageExpand");
    if (expandImage != null) {
	img.src = expandImage;
        img.alt = netUI.netUITree.trees[treeName].altCollapse;
    }
    else {
	if (expandLast != null) {
...
function NetUICollapseTreeNode(node,raiseCommand)
{
    ...
    if (raiseCommand) {
       ...
       var ajaxPrefix = netUI.netUITree.trees[treeName].ajaxPrefix;
       var ajaxParam =  netUI.netUITree.trees[treeName].ajaxParam;
       var url = netUI.netUITree.getTreeCommandUrl("treeCollapse",treeName,
		   nodeName,"false",ajaxPrefix,ajaxParam);
       netUI.raiseCommand(url,NetUITreeXmlHttpRequestReturn);
    }
    ...
    if (expandImage != null) {
	img.src = expandImage;
        img.alt = netUI.netUITree.trees[treeName].altExpand;
    }
    else {
	if (nodeName != null) {
